...
<script>
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            var modalBody = `
                <form id="modal-form" class="edit-modal" action="{{ route('office.psychologist.appointment.update', ['id' => 0, 'slug' => $office->slug]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT') 
                ...
                <div style="width:100%;display:flex;align-items:center;justify-content:end;padding: 20px;gap:20px;">
                  ...
                  <a class="btn btn-danger" id="delete-button" href="{{ route('office.psychologist.appointment.destroy', ['id' => 0, 'slug' => $office->slug]) }}">Sil</a>
            ...
            let action = '{{ route('office.psychologist.appointment.update', ['id' => 0, 'slug' => $office->slug]) }}'

            action = action.replace('/0', '/' + info.event.extendedProps.appointment_id)

            let href = "{{ route('office.psychologist.appointment.destroy', ['id' => 0, 'slug' => $office->slug]) }}"
        ...
        dateClick: function(info) {
            ...
            var modalBody = `
                <form class="edit-modal" action="{{ route('office.psychologist.appointment.store', ['slug' => $office->slug]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="input-container">
                    <label for="client_name" class="input-label">Danışan İsim</label>
