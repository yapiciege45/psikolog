...
@section('content')
    ...
    <form action="{{ route('office.dashboard.appointment.store', ['slug' => $office->slug]) }}" method="POST" enctype="multipart/form-data" class="modal fade" id="createModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        @csrf
                ...
                <tr>
                    ...
                    <td>
                        <div class="table-actions">
                            ...
                            <a href="{{ route('office.dashboard.appointment.destroy', ['id' => $appointment->id, 'slug' => $office->slug]) }}"  class="delete-action">
    ...
    @foreach ($appointments as $appointment)
        <form action="{{ route('office.dashboard.appointment.update', ['id' => $appointment->id, 'slug' => $office->slug]) }}" enctype="multipart/form-data" method="POST" class="modal fade" id="editModal-{{ $appointment->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editModal-{{ $appointment->id }}Label" aria-hidden="true">
            @csrf
            @method('PUT')
                ...
                <div class="modal-body edit-modal">
                    <div class="input-container">
                        ...
                        <select name="user_id" id="user_id">
                            <option value="0">Seçiniz</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected($user->id == $appointment->user_id)>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="input-container">
