...
@section('content')
    ...
    <form action="{{ route('office.dashboard.assistant.store', ['slug' => $office->slug]) }}" method="POST" enctype="multipart/form-data" class="modal fade" id="createModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        @csrf
                ...
                <tr>
                    ...
                    <td>
                        <div class="table-actions">
                            ...
                            <a href="{{ route('office.dashboard.assistant.destroy', ['id' => $assistant->id, 'slug' => $office->slug]) }}"  class="delete-action">
    ...
    @foreach ($assistants as $assistant)
        <form action="{{ route('office.dashboard.assistant.update', ['id' => $assistant->id, 'slug' => $office->slug]) }}" enctype="multipart/form-data" method="POST" class="modal fade" id="editModal-{{ $assistant->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editModal-{{ $assistant->id }}Label" aria-hidden="true">
            @csrf
            @method('PUT')
                ...
                <div class="modal-body edit-modal">
                    ...
                    <div class="input-container">
                        ...
                        <select 
                            ...
                            id="office_select_{{$assistant->id}}"
                            multiple
                        >
                            @foreach ($office_branches as $branch)
