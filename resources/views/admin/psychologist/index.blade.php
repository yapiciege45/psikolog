...
@section('content')
    ...
    <form action="{{ route('dashboard.psychologist.store') }}" method="POST" enctype="multipart/form-data" class="modal fade" id="createModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        @csrf
                ...
                <tr>
                    ...
                    <td>
                        <div class="table-actions">
                            ...
                            <a href="{{ route('dashboard.psychologist.destroy', ['id' => $psychologist->id]) }}"  class="delete-action">
    ...
    @foreach ($psychologists as $psychologist)
        <form action="{{ route('dashboard.psychologist.update', ['id' => $psychologist->id]) }}" enctype="multipart/form-data" method="POST" class="modal fade" id="editModal-{{ $psychologist->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editModal-{{ $psychologist->id }}Label" aria-hidden="true">
            @csrf
            @method('PUT')
                ...
                <div class="modal-body edit-modal">
                    ...
                    <div class="input-container">
                        ...
                        <select 
                            ...
                            data-selected-branch-ids='[@foreach ($psychologist->office->officeBranches as $branch) "{{$branch->id}}" @if(!$loop->last),@endif @endforeach]' 
                            data-selected-office-id="{{ $psychologist->office->id }}" 
                            data-id="{{ $psychologist->id }}" 
                            id="office_branch_update_id_{{ $psychologist->id }}" 
                            multiple
                        >
